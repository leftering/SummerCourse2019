...
fdata = open("C:/Users/Dell/Desktop/SummerCourse2019/email-Eu-core-temporal-Dept1.txt")
...
while flag:
    ...
    while i < len(datalist):
        if i < len(datalist) and int(datalist[i][2]) <= startt + 60*60*24*30 and int(datalist[i][2]) >= startt:
            dataframes[j].append(datalist[i])
            if i == len(datalist)-1:
                flag = False
        i = i + 1
    j = j + 1
    startt = startt + 60*60*24*3
...
while i < len(dataframes):
    ...
    while p < len(dataframes[i]):
        w = G.get_edge_data(dataframes[i][p][0], dataframes[i][p][1])
        G.add_weighted_edges_from([(dataframes[i][p][0], dataframes[i][p][1],1 if w == None else w["weight"] + 1)])
        p = p + 1
    # nx.draw(G, with_labels=True, font_size=8, node_size=30)
    # print(G.nodes())
    # print(G.edges())
...
while i < len(newx):
    ...
    while j < len(dataframes[i]):
        w = G.get_edge_data(dataframes[i][j][0], dataframes[i][j][1])
        G.add_node(dataframes[i][j][0], cls=dataframes[i][j][0])
        G.add_node(dataframes[i][j][1], cls=dataframes[i][j][1])
        G.add_weighted_edges_from([(dataframes[i][j][0], dataframes[i][j][1],1 if w == None else w["weight"] + 1)])
        j = j + 1
    nodes = G.nodes()
    for no in nodes:
